...
This module defines 2 functions to help use the Munit test framework.
...
.. command:: munit_get_test_list

  Get list of all tests from from a Munit executable.

    munit_get_test_list(executable
                        list_of_tests_variable
                        [ERRNO_VARIABLE var]
                        [OUTPUT_VARIABLE var]
                        [WORKING_DIRECTORY dir]
    )
  ...
  ``list_of_tests_variable``
    Variable to write the list of discovered tests to.

  ``ERRNO_VARIABLE var``
    Variable to write the exit code of ``executable --list`` to.

  ``OUTPUT_VARIABLE var``
    Variable to write the raw output of ``executable --list`` to.

  ``WORKING_DIRECTORY dir``
    Working directory to use when executing the above mentioned commands.
...
function (munit_get_test_list EXECUTABLE LIST_OF_TESTS)
  cmake_parse_arguments(
    ""
    ""
    "OUTPUT_VARIABLE;ERRNO_VARIABLE;WORKING_DIRECTORY"
    ""
    ${ARGN}
  )

  execute_process(
    COMMAND "${EXECUTABLE}" --list
